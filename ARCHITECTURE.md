...
| 0.9.0 | 2026-06-26 | + `AssertionEntry` TimelineEntry variant (assertion checkpoint in recording timeline). + `Recording.parameters: list[ParameterDeclaration]` (named params for replay parametrization, additive). + `RecordingsState.active_replay_progress: ReplayProgress \| None` (live replay progress, additive). New models: `ReplayReport`, `ReplayStepResult`, `ReplayReportMeta`, `ReplayProgress`, `ReplayStatus`. SQLite table: `replay_reports` (JSON blobs for `parameters` + `step_results`). StateManager: `add_assertion_to_timeline`, `delete_assertion`, `update_assertion`, `add_parameter_to_recording`, `save_replay_report`, `get_replay_report`, `list_replay_reports_meta`, `set_active_replay_progress`. |
| 0.10.0 | 2026-06-26 | + voice-over extension. `TranscriptSegmentEntry` (7th TimelineEntry variant). Voice-over fields on `Recording`/`RecordingMeta`: `has_voice_over`, `audio_path`, `transcription_status`, `transcription_error`. New state subtrees on `StateSnapshot`: `microphone_state: MicrophoneState`, `settings_state: SettingsState`, `transcription_state: TranscriptionState`. New models: `MicrophoneDevice`, `MicrophoneState`, `SettingsState`, `TranscriptionBackendStatus`, `TranscriptionBackendInfo`, `TranscriptionState`. SQLite: `settings` key-value table (new); `recordings` table: 4 additive columns (`has_voice_over`, `audio_path`, `transcription_status`, `transcription_error`) via ALTER TABLE migration guard. StateManager: `set_audio_path`, `set_transcription_status`, `append_transcript_segments` (PIT-105 batch pattern), `update_microphone_state` (topology-hash guard), `set_mic_device`, `set_settings`, `update_transcription_backend_status`. |
| 0.11.0 | 2026-06-27 | + model-catalog extension. New model: `TranscriptionModelSpec` (frozen, fields: `model_id`, `display_name`, `hf_repo_id`, `default`). Extended `SettingsState` with `mlx_whisper_model_id: str \| None`. Extended `TranscriptionBackendInfo` with `available_models: list[TranscriptionModelSpec]` and `selected_model_id: str \| None`. `MODEL_CATALOG` in `mlx_whisper.py` (2 entries: whisper-base-mlx default + whisper-large-v3-turbo). `set_model()` no-op default on `TranscriptionBackend` Protocol (COL-6 SSoT). Persistence: `save_mlx_whisper_model_id` / `load_mlx_whisper_model_id` in protocol + in_memory + sqlite. StateManager: `set_mlx_whisper_model()` (COL-2 re-probe, COL-3 selected_model_id sync). `build_initial_transcription_state()` in show_session.py extended with `available_models` + `selected_model_id`. |
